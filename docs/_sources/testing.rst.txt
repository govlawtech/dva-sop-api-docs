...
2. Upload test files to the folder at https://github.com/govlawtech/dva-sop-api/tree/dvatesting/system-tests/src/test/resources/dvaDefinedTestData
...
In order to report on these tests cleanly, the results for these tests are collected into a separate HTML file. This HTML file is automatically posted to an Azure function app that stores it for later retrieval.
The latest test report can be obtained from: https://functions-dva-sop-api.azurewebsites.net/api/SystemTests?code=oqcdXdK3DF/cZcWNvsMBXJauaGbmgeJww2RK00GcXy7S1mjTKaKDmg==
